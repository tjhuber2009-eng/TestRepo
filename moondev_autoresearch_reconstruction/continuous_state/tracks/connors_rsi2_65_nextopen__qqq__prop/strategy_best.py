...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 220:
            return
        px=float(self.data.Close[-1]); sma=float(self.sma200[-1]); r=float(self.rsi2[-1]); rv=float(self.rv[-1]); atr=float(self.atr[-1])
        if not np.isfinite([px,sma,r,rv,atr]).all() or rv <= 0 or atr <= 0:
            return
        if not self.position and px > sma and r < 5:
            self._buy_with_stop(px, atr, stop_mult=3.0)
