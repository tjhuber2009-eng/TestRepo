...
class MoonStrategy(Strategy):
    ...
    def next(self):
        ...
        if not np.isfinite([px,lo,hi,prior,avg,rv,adx_val]).all() or hi <= lo or rv <= 0:
            return
        ibs=(px-lo)/(hi-lo)
        lower=prior - 2.5*avg
        if not self.position and px < lower and ibs < 0.30 and adx_val > 20:
