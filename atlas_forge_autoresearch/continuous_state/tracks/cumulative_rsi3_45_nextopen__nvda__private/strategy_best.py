...
class MoonStrategy(Strategy):
    ...
    def next(self):
        ...
        if not np.isfinite([px,r,s,rv]).all() or rv <= 0:
            return
        # conceptual change: require a down day (close below prior close) for entry
        if not self.position and s < 45 and self.data.Close[-1] < self.data.Close[-2]:
