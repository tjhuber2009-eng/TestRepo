...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rv_ma = self.I(_sma_now, self.rv, 20)

    def next(self):
        if len(self.data.Close) < 50:
            return
        px=float(self.data.Close[-1]); r=float(self.rsi2[-1]); s=float(self.rsi3sum[-1]); rv=float(self.rv[-1]); rv_ma=float(self.rv_ma[-1])
        if not np.isfinite([px,r,s,rv,rv_ma]).all() or rv <= 0:
            return
        if not self.position and s < 45 and rv < rv_ma:
