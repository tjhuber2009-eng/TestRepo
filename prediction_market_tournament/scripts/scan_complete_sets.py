...
from tournament.complete_set_market import is_neg_risk_multioutcome_event, snapshot_complete_set
...
def main() -> int:
    ...
    candidates = [e for e in events if is_neg_risk_multioutcome_event(e)]
    opportunities = 0
    complete = 0
    errors: list[str] = []
    for event in candidates:
        ...
        if snap is None:
            continue
        complete += 1
        row = {"spec_sha256": sha, **snap.as_json()}
    ...
    append_jsonl(scan_log, {
        ...
        "errors": errors[:50],
        "note": "shadow lane: gross edge only; no promotion until per-leg fees/fill simultaneity are modeled",
    })
    print(json.dumps({
        "active_events": len(events),
        "neg_risk_multioutcome_events": len(candidates),
