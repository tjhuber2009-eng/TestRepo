...
def data():
    ...
    # Use early all-time mirror only where an exchange feed is not available; use Bitstamp from 2013 onward.
    x=pd.concat([early.loc[:'2012-12-31'],bs]).sort_index()
    x=x[~x.index.duplicated(keep='last')]
    return x.loc[:CURRENT_END]


def nearest_bar(idx, target, direction='on_or_after'):
    target=pd.Timestamp(target)
    if direction=='on_or_after':
        p=idx.searchsorted(target,side='left')
        return idx[min(p,len(idx)-1)]
    p=idx.searchsorted(target,side='right')-1
    return idx[max(p,0)]
...
def schedule(df, pre_days, post_days):
    ev=[]
    for h in HALVINGS:
        en=nearest_bar(df.index,h-pd.Timedelta(days=pre_days),'on_or_after')
        ex=nearest_bar(df.index,h+pd.Timedelta(days=post_days),'on_or_after')
        ev.append((en,ex,h))
    return ev


def px(df, dt, mode, side):
...
def bt(df, pre_days, post_days, mode='next_open', end='2025-01-31', fee_side=0.0, force_exit=True):
    end=pd.Timestamp(end); d=df.loc[:end].copy(); ev=schedule(df,pre_days,post_days)
    cash=INITIAL; qty=0.0; trades=[]; curve=[]; active=None
    actions={}
    for en,ex,h in ev:
    ...
    for dt,row in d.iterrows():
        for act,h in actions.get(dt,[]):
            p,filldt=px(df,dt,mode,act)
            if filldt>end: continue
            if act=='buy' and qty==0:
                cap=cash; qty=cash/(p*(1+fee_side)); cash-=qty*p*(1+fee_side)
                active={'entry':filldt,'entry_px':p,'entry_cap':cap,'halving':h}
            elif act=='sell' and qty>0:
                cash += qty*p*(1-fee_side)
                trades.append({**active,'exit':filldt,'exit_px':p,'pnl':cash-active['entry_cap'],'ret_pct':(cash/active['entry_cap']-1)*100})
                qty=0.0; active=None
        curve.append((dt,cash+qty*float(row.Close)))
    if force_exit and qty>0:
        dt=d.index[-1]; p=float(d.iloc[-1].Close)
        cash += qty*p*(1-fee_side)
        trades.append({**active,'exit':dt,'exit_px':p,'pnl':cash-active['entry_cap'],'ret_pct':(cash/active['entry_cap']-1)*100,'forced_exit':True})
        qty=0.0; curve[-1]=(dt,cash)
    tr=pd.DataFrame(trades); eq=pd.Series(dict(curve)).sort_index()
    final=float(eq.iloc[-1]); ret=(final/INITIAL-1)*100
    ...
    return {'final':final,'return_pct':ret,'cagr_pct':(final/INITIAL)**(1/years)*100-100,'trades':len(tr),'wins':wins,'win_pct':100*wins/len(tr) if len(tr) else np.nan,'profit_factor':gp/gl if gl>0 else np.inf,'max_dd_close_pct':float(dd.min()),'start':str(eq.index[0].date()),'end':str(eq.index[-1].date())},tr,eq
...
def main():
    out='audit_output_halving'; os.makedirs(out,exist_ok=True)
    df=data(); print('DATA',df.index.min(),df.index.max(),len(df))
    rows=[]
    # Public/common and interpretable candidate windows.
    ...
    for pre,post,label in candidates:
        for mode in ['same_open','same_close','next_open']:
            for fee_name,fee in [('zero',0.0),('15bps_side',0.0015),('50bps_side',0.005)]:
                m,tr,eq=bt(df,pre,post,mode,'2025-01-31',fee_side=fee)
                rows.append({'kind':'candidate','label':label,'pre_days':pre,'post_days':post,'mode':mode,'fee':fee_name,**m,'claim_ratio':m['final']/CLAIM_FINAL})
                if label=='approx_2y_18m' and mode=='next_open' and fee_name=='zero': tr.to_csv(f'{out}/two_year_18m_trades.csv',index=False)
    # Reverse-engineering grid. This is explicitly NOT validation; it measures curve-fitability.
    fits=[]
    for mode in ['same_open','same_close','next_open']:
        for pre in range(360,901,10):
            for post in range(360,701,10):
                m,_,_=bt(df,pre,post,mode,'2025-01-31',0.0)
                if m['trades']!=4: continue
                err=abs(math.log(max(m['final'],1)/CLAIM_FINAL))
                fits.append({'mode':mode,'pre_days':pre,'post_days':post,**m,'log_final_error':err,'claim_ratio':m['final']/CLAIM_FINAL})
    fits=pd.DataFrame(fits).sort_values('log_final_error')
    fits.head(100).to_csv(f'{out}/offset_grid_top100.csv',index=False)
    # Search plausible late-Jan 2025 snapshot with the simple ~2y/~18m window.
    snaps=[]
    for e in pd.date_range('2025-01-01','2025-03-31',freq='D'):
        for mode in ['same_open','same_close','next_open']:
            m,_,_=bt(df,730,548,mode,e,0.0)
            if m['trades']==4:
                snaps.append({'mode':mode,'end':str(e.date()),**m,'claim_ratio':m['final']/CLAIM_FINAL,'abs_pct_gap':abs(m['final']/CLAIM_FINAL-1)*100})
    snaps=pd.DataFrame(snaps).sort_values('abs_pct_gap')
    snaps.head(100).to_csv(f'{out}/snapshot_search_top100.csv',index=False)
    # Timing sensitivity around an interpretable 730/548 rule, fixed Jan31 snapshot.
    sens=[]
    for pre in range(670,791,10):
        for post in range(488,609,10):
            m,_,_=bt(df,pre,post,'next_open','2025-01-31',0.0)
            sens.append({'pre_days':pre,'post_days':post,**m})
    pd.DataFrame(sens).to_csv(f'{out}/timing_sensitivity.csv',index=False)
    # Current completed result of each fixed candidate through current date.
    current=[]
    for pre,post,label in candidates:
        for mode in ['next_open']:
            m,tr,eq=bt(df,pre,post,mode,CURRENT_END,0.0)
            current.append({'label':label,'pre_days':pre,'post_days':post,'mode':mode,**m})
            tr.to_csv(f'{out}/current_{label}_trades.csv',index=False)
    # Exact benchmark windows for headline candidates.
    frame=pd.DataFrame(rows)
    frame.to_csv(f'{out}/candidate_results.csv',index=False)
    pd.DataFrame(current).to_csv(f'{out}/current_results.csv',index=False)
    best=fits.iloc[0].to_dict(); bestsnap=snaps.iloc[0].to_dict()
    # Pull key zero-cost rows for compact summary.
    key={}
    for label in [x[2] for x in candidates]:
