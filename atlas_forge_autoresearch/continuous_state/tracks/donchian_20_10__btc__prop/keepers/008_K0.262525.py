...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rsi_now = self.I(_rsi_now, self.data.Close, 14)

    def next(self):
        if len(self.data.Close) < 40:
            return
        px=float(self.data.Close[-1]); hh=float(self.hh[-1]); ll=float(self.ll[-1]); atr=float(self.atr[-1])
        if not np.isfinite([px,hh,ll,atr,float(self.rv[-1]),float(self.rsi_now[-1])]).all() or atr <= 0:
            return
        if not self.position and px > hh and self.rsi_now[-1] < 70:
