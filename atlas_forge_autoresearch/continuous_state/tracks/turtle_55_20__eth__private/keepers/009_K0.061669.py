...
class MoonStrategy(Strategy):
    ...
    def next(self):
        ...
        if not self.position and px > hh:
            ...
            self.high_since_entry = self.data.High[-2]  # high of the bar just completed
        elif self.position:
            # update highest high since entry using prior bar's high
            prev_high = self.data.High[-2]
            self.high_since_entry = max(self.high_since_entry, prev_high)
            stop = self.high_since_entry - 2.0 * atr
