...
class AtlasStrategy(Strategy):
    ...
    vol_target = 0.3
    f_max = 2.0
    profit_target = 3.0  # profit target as multiple of ATR
    ...
    def init(self):
        ...
        self.entry_price = None  # track entry price for profit target
    ...
    def next(self):
        ...
        if not self.position and px > hh:
            ...
            # exit if price breaks Donchian low or hits profit target
            if px < ll or (self.entry_price is not None and px >= self.entry_price + self.profit_target * atr):
                self.position.close()
