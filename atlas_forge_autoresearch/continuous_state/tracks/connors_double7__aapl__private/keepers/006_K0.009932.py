...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_avg = self.I(_vol_mean, self.data.Volume, 20)
    ...
    def next(self):
        ...
        if not self.position and px > s200 and px <= lo + eps and self.data.Volume[-1] > self.vol_avg[-1]:
