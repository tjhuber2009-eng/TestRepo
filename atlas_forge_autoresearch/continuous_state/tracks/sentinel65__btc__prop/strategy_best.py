...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_ma = self.I(_vol_mean, self.data.Volume, self.vol_lookback)
    ...
    def next(self):
        ...
        vol_ma_val = float(self.vol_ma[-1])
        if not np.isfinite([vol, vol_ma_val]).all() or vol_ma_val <= 0:
            return
        z=(px-ema)/sd
        if not self.position and z > 0.5 and vol > vol_ma_val:
