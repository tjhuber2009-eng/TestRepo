...
PUBLIC_GUIDE_APPROX='2026-05-09'
...
def linreg(src,n,offset=0):
    ...
    for i in range(n-1,len(y)):
        ...
        ym=z.mean(); slope=((x-xm)*(z-ym)).sum()/den; intercept=ym-slope*xm; out[i]=intercept+slope*(n-1-offset)
    return pd.Series(out,index=src.index)

def make_state(df,fast=14,slow=42,signal_n=13,trend_n=19,rsi_n=6,exit_variant='opposite_all'):
    c=df.Close.astype(float); macd=pine_ema(c,fast)-pine_ema(c,slow); sig=pine_ema(macd,signal_n); tr=linreg(c,trend_n); rsi=pine_rsi(c,rsi_n)
    turn_up=(sig>sig.shift(1))&(sig.shift(1)<=sig.shift(2)); turn_dn=(sig<sig.shift(1))&(sig.shift(1)>=sig.shift(2))
...
def metrics_row(source,df,state,mode,cost,window,start,end,slow,exit_variant):
    ...
    return {'source':source,'window':window,'slow':slow,'exit_variant':exit_variant,'mode':mode,'cost_side_bps':cost*10000,'final_from_10000':10000*(1+m['total_return_pct']/100),**m},eng

def stitch(early,bit):
    a=early.loc[:'2012-12-31',['Open','High','Low','Close']].copy(); b=bit.loc['2013-01-01':,['Open','High','Low','Close']].copy(); x=pd.concat([a,b]).sort_index(); return x[~x.index.duplicated(keep='last')]
...
def capacity(df,eng):
    ...
    for _,t in eng.trades.iterrows():
        ...
        eq=float(t.entry_equity_pre_fee)*10000; vol=float(df.loc[dt].get('Volume',np.nan)); cap=float(df.loc[dt].get('Market Cap',np.nan))
        rows.append({'date':dt,'notional_usd':eq,'daily_volume':vol,'market_cap':cap,'pct_daily_volume':100*eq/vol if np.isfinite(vol) and vol>0 else np.nan,'pct_market_cap':100*eq/cap if np.isfinite(cap) and cap>0 else np.nan})
    return pd.DataFrame(rows)

def main():
    os.makedirs('audit_output_momentummagic',exist_ok=True); early=fetch_early(); bit=fetch_bitstamp(); hybrid=stitch(early,bit); rows=[]; default_eng=None; costs=[0.0,0.0015,0.005]
      ...
      for slow in [42,72]:
       for exit_variant in ['opposite_all','signal_turn']:
        ...
        for mode in ['same_close','next_open','next_close']:
         for cost in costs:
          windows=[('claim_full',CLAIM_START,CLAIM_END),('2013_claim','2013-01-01',CLAIM_END),('2018_claim','2018-01-01',CLAIM_END),('2022_claim','2022-01-01',CLAIM_END)] if source.startswith('hybrid') else [('bitstamp_2013_claim','2013-01-01',CLAIM_END),('post_current_version',PUBLIC_GUIDE_APPROX,CURRENT_END),('2023_current','2023-01-01',CURRENT_END),('2024_current','2024-01-01',CURRENT_END)]
          eng=None
          for w,ws,we in windows:
              rr,eng=metrics_row(source,df,state,mode,cost,w,ws,we,slow,exit_variant)
              if rr: rr['closed_trade_only_dd_pct']=closed_trade_dd(eng,ws,we); rows.append(rr)
          if source.startswith('hybrid') and slow==42 and exit_variant=='opposite_all' and mode=='next_open' and cost==0: default_eng=eng; eng.trades.to_csv('audit_output_momentummagic/default_trades.csv',index=False)
    out=pd.DataFrame(rows); out.to_csv('audit_output_momentummagic/results.csv',index=False)
    bh=[]
    for source,d,windows in [('hybrid_early_plus_bitstamp',hybrid,[('claim_full',CLAIM_START,CLAIM_END),('2013_claim','2013-01-01',CLAIM_END),('2018_claim','2018-01-01',CLAIM_END)]),('bitstamp',bit,[('post_current_version',PUBLIC_GUIDE_APPROX,CURRENT_END),('2023_current','2023-01-01',CURRENT_END),('2024_current','2024-01-01',CURRENT_END)])]:
        for w,ws,we in windows:
    ...
    print('=== MOMENTUMMAGIC CLAIM TARGET ==='); print(json.dumps(TARGET,indent=2)); print('\n=== PRIMARY RECONSTRUCTION ==='); print(focus[['slow','mode','cost_side_bps','total_return_pct','cagr_pct','max_dd_pct','closed_trade_only_dd_pct','profit_factor','closed_trades','winning_trades']].to_string(index=False,float_format=lambda x:f'{x:.3f}'))
    print('\n=== RECENT / FORWARD ==='); print(recent[['window','cost_side_bps','total_return_pct','cagr_pct','max_dd_pct','profit_factor','closed_trades','winning_trades']].to_string(index=False,float_format=lambda x:f'{x:.3f}')); print('\n=== SLOW DIAGNOSTIC TOP 20 ==='); print(diag[['slow','mode','score','total_return_pct','cagr_pct','max_dd_pct','closed_trade_only_dd_pct','profit_factor','closed_trades','winning_trades']].head(20).to_string(index=False,float_format=lambda x:f'{x:.3f}')); print('\n=== BUY HOLD ==='); print(pd.DataFrame(bh).to_string(index=False,float_format=lambda x:f'{x:.3f}'))
    if len(cap): print('\n=== CAPACITY ==='); print(cap.sort_values('date').tail(15).to_string(index=False,float_format=lambda x:f'{x:.4f}'))
