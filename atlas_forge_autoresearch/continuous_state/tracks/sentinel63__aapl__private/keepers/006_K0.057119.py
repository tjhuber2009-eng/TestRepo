...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 80:
            return
        px=float(self.data.Close[-1]); ema=float(self.ema63[-1]); sd=float(self.sd63[-1]); rv=float(self.rv[-1]); atr_val=float(self.atr[-1])
        if not np.isfinite([px,ema,sd,rv,atr_val]).all() or sd <= 0 or rv <= 0 or atr_val <= 0:
            return
        z=(px-ema)/sd
        if not self.position and z > 0.5:
            ...
            if px > self.max_price:
                self.max_price = px
            if px < self.max_price - 2.0 * atr_val:
                self.position.close()
                self.max_price = 0.0
