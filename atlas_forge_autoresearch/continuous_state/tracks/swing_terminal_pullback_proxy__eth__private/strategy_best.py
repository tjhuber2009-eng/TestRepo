...
class MoonStrategy(Strategy):
    ...
    def next(self):
        ...
        if not np.isfinite([px,e20,e50,atr,float(self.rv[-1]),float(self.rsi[-1])]).all() or atr <= 0:
            return
        trend = e20 > e50 and px > e50
        pullback = abs(px - e20) <= 0.40 * atr
        if not self.position:
            self.high_since_entry = 0.0
            if trend and pullback and self.rsi[-1] > 50:
                self._buy_with_stop(px, atr, 1.5)
                self.high_since_entry = px
        else:
            if px > self.high_since_entry:
                self.high_since_entry = px
            if px < self.high_since_entry - 2.0 * atr:
                self.position.close()
