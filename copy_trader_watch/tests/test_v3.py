...
def snap(platform="x", actionable=False, free=True, us="yes", score=10):
    s = TraderSnapshot(platform=platform, trader_id="id", name="Name", observed_at="2026-09-03T00:00:00Z", source="test", free=free, us_access=us, live_evidence="real", return_pct=10, return_window="month", max_drawdown_pct=-5, actionable=actionable, actionable_reason="reason")
    s.research_score = score
    return s


def test_all_records_sorts_score():
    rows = run_v3.all_records([AdapterResult("a", "x", [snap(score=5), snap(score=20)])])
    assert rows[0].research_score == 20


def test_report_separates_actionable_and_research():
    ...
    report = run_v3.build_report([AdapterResult("etoro", "x", [a]), AdapterResult("hyperliquid", "x", [b])], {"report": {"top_n": 10}})
    assert "Free U.S.-actionable candidates" in report
