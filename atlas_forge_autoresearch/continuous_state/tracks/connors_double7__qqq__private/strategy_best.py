...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.adx = self.I(_adx_now, self.data.High, self.data.Low, self.data.Close, 14)
        self.vol_mean = self.I(_vol_mean, self.data.Volume, 20)

    def next(self):
        if len(self.data.Close) < 220:
            return
        px=float(self.data.Close[-1]); s200=float(self.sma200[-1]); lo=float(self.low7[-1]); hi=float(self.high7[-1]); rv=float(self.rv[-1]); adx=float(self.adx[-1]); vol=float(self.data.Volume[-1]); vol_mean=float(self.vol_mean[-1])
        if not np.isfinite([px,s200,lo,hi,rv,adx,vol,vol_mean]).all() or rv <= 0:
            return
        eps=max(1e-12,abs(px)*1e-12)
        if not self.position and px > s200 and px <= lo + eps and adx > 20 and vol > vol_mean:
