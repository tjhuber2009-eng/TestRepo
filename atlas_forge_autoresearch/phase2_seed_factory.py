"""Isolated Phase-2 signal library recovered from prior indicator-tournament work.

This module is deliberately NOT part of strategy_library/registry.json, so the
frozen 514-track Phase-1 universe cannot change while Phase 2 runs in parallel.

Signal logic follows the user's prior tournament implementation. Execution is
translated to the project's causal next-bar convention and existing
volatility/risk profile so fills are comparable across AUTORESEARCH.
"""
...
FAMILIES = [
    ...
    "roc12_zero",
    "momentum10",
    "rsi14_momentum_55_45",
    "rsi14_reversion_30_70",
    "stochastic_14_3_3",
    "stochrsi_14",
    "utbot_key1_atr10",
    "sma_10_50",
    ...
    "cci20_100",
    "williams_r14",
    "bollinger_breakout_20_2",
    "keltner_breakout_20_2atr",
    "donchian_50_state",
    "supertrend_14_2",
    "chandelier_22_3",
    "dmi_adx14_20",
...
def generate(family: str, output: str | Path, bars_per_year: int, vol_target: float, f_max: float) -> Path:
    if family not in FAMILIES:
        raise ValueError(f"unknown phase2 family: {family}")
    # Heikin-Ashi entries require Open and are intentionally kept out until a
    # 5-argument signal adapter is added; do not silently approximate them.
    if family in {"heikin_ashi_color", "ha_ema50"}:
        raise ValueError(f"{family}: requires exact OHLC Heikin-Ashi adapter")
    source = (
        SOURCE_TEMPLATE
        .replace("__FAMILY__", family)
        .replace("__BARS_PER_YEAR__", str(int(bars_per_year)))
        .replace("__VOL_TARGET__", repr(float(vol_target)))
        .replace("__F_MAX__", repr(float(f_max)))
    )
    path = Path(output)
...
def runnable_families() -> list[str]:
    return [x for x in FAMILIES if x not in {"heikin_ashi_color", "ha_ema50"}]
...
def main() -> None:
    ...
    args=ap.parse_args()
    path=generate(args.family,args.output,args.bars_per_year,args.vol_target,args.f_max)
    print(f"generated Phase-2 {args.family} -> {path}")
