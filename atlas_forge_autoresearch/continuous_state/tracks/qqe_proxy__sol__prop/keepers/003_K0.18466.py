...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 60:
            return
        px=float(self.data.Close[-1]); r=float(self.rsi[-1]); rm=float(self.rsi_ma[-1]); atr=float(self.atr[-1]); sma50_val=float(self.sma50[-1])
        if not np.isfinite([px,r,rm,atr,float(self.rv[-1]),sma50_val]).all() or atr <= 0:
            return
        if not self.position and r > 55 and r > rm and px > sma50_val:
