...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_mean = self.I(_vol_mean, self.data.Close, 60)  # 60-day mean volume for volatility scaling

    def next(self):
        if len(self.data.Close) < 50:
            return
        px=float(self.data.Close[-1]); r=float(self.rsi2[-1]); s=float(self.rsi3sum[-1]); rv=float(self.rv[-1]); volm=float(self.vol_mean[-1])
        if not np.isfinite([px,r,s,rv,volm]).all() or rv <= 0 or volm <= 0:
            return
        vol_scaled = rv / volm
        vol_scaled = max(0.5, min(2.0, vol_scaled))  # cap between 0.5 and 2.0
        dynamic_exit = 65 + (vol_scaled * 10)  # volatility-adjusted exit threshold
        if not self.position and s < 40:
            units=self._units(px,rv)
            if units >= 1:
                self.buy(size=units)
        elif self.position and r > dynamic_exit:
            self.position.close()
