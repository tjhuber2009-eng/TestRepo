...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rv_ma = self.I(_sma, self.rv, 60)  # 60‑day SMA of realized vol

    def next(self):
        if len(self.data.Close) < 60:  # need enough data for rv_ma
            return
        ...
        if not np.isfinite([px,ma,sd,atr,float(self.rv[-1])]).all() or sd <= 0 or atr <= 0:
            return
        if not self.position and px < ma - 2.0 * sd and self.rv[-1] < self.rv_ma[-1]:
