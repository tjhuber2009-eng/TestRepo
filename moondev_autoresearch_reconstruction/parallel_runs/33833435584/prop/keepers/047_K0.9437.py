...
class MoonStrategy(Strategy):
    ...
    entry_lookback = 20
    exit_lookback = 4  # Decreased from 5 to 4 for more responsive exits
    atr_lookback = 20
    stop_atr = 3.5  # Increased from 3.0 to reduce premature stop outs
    exit_atr_mult = 0.10  # Decreased from 0.15 to exit earlier and reduce drawdown
    ...
    def init(self):
        ...
        # Volume average for entry filter
        self.vol_avg = self.I(
            lambda x: pd.Series(x).rolling(20).mean().shift(1).to_numpy(),
            self.data.Volume
        )
    ...
    def next(self):
        ...
        rv = float(self.rv[-1])
        vol_avg = float(self.vol_avg[-1])
        vol = float(self.data.Volume[-1])

        if not np.isfinite([px, hh, ll, atr, rv, vol_avg, vol]).all() or atr <= 0:
            return

        if not self.position:
            if px > hh:
                # Require breakout bar to close in upper half of its range
                if self.data.Close[-1] > (self.data.High[-1] + self.data.Low[-1]) / 2:
                    # Require volume above 0.9 * 20-bar average
                    if vol > 0.9 * vol_avg:
        ...
        elif px < ll - self.exit_atr_mult * atr and self.data.Close[-1] < (self.data.High[-1] + self.data.Low[-1]) / 2 and vol > vol_avg:
            self.position.close()
