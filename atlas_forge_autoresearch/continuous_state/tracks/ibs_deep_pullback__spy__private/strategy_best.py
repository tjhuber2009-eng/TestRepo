...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_ma = self.I(_sma_now, self.rv, self.vol_ma_lookback)
    ...
    def next(self):
        ...
        entry_allowed = np.isfinite(vol_ma_val) and rv < vol_ma_val
        if not self.position and px < lower and ibs < 0.30 and entry_allowed:
