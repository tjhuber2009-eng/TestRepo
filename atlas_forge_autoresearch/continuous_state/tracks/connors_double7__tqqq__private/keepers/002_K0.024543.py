...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 220:
            return
        px=float(self.data.Close[-1]); s200=float(self.sma200[-1]); lo=float(self.low7[-1]); hi=float(self.high7[-1]); rv=float(self.rv[-1]); adx_val=float(self.adx[-1])
        if not np.isfinite([px,s200,lo,hi,rv,adx_val]).all() or rv <= 0:
            return
        eps=max(1e-12,abs(px)*1e-12)
        if not self.position and px > s200 and px <= lo + eps and adx_val > 20:
