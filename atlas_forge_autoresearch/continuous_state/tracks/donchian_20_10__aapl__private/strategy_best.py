...
class MoonStrategy(Strategy):
    ...
    vol_target = 0.3
    f_max = 2.0
    trail_atr_mult = 3.0   # ATR multiplier for trailing stop
    ...
    def next(self):
        ...
        if not self.position and px > hh:
            ...
            # initialize trailing stop and highest high since entry
            self.trailing_stop = px - atr * self.trail_atr_mult
            self.highest_since_entry = px
        elif self.position:
            # update highest high since entry
            high_today = float(self.data.High[-1])
            if high_today > self.highest_since_entry:
                self.highest_since_entry = high_today
            # recompute trailing stop
            new_stop = self.highest_since_entry - atr * self.trail_atr_mult
            if new_stop > self.trailing_stop:
                self.trailing_stop = new_stop
            # exit if price hits trailing stop
            if px <= self.trailing_stop:
                self.position.close()
