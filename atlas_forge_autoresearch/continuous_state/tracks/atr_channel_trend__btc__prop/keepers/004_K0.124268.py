...
class MoonStrategy(Strategy):
    vol_lookback = 30
    vol_target = 0.04299237
    f_max = 0.5
    trail_mult = 2.5   # ATR multiplier for trailing stop
    ...
    def init(self):
        ...
        self.trailing_high = None   # track highest close since entry
    ...
    def next(self):
        ...
        if not self.position:
            if px > ema + 1.5 * atr:
                ...
                self.trailing_high = px
        else:
            # update trailing high
            if self.trailing_high is None or px > self.trailing_high:
                self.trailing_high = px
            trailing_stop = self.trailing_high - self.trail_mult * atr
            if px < trailing_stop:
                self.position.close()
