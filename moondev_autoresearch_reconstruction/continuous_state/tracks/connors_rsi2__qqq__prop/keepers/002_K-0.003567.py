...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_ma = self.I(_sma, self.rv, 20)

    def next(self):
        if len(self.data.Close) < 220:
            return
        px=float(self.data.Close[-1]); s200=float(self.sma200[-1]); s5=float(self.sma5[-1]); r=float(self.rsi2[-1]); rv=float(self.rv[-1]); vol_ma=float(self.vol_ma[-1]) if len(self.vol_ma) else np.nan
        if not np.isfinite([px,s200,s5,r,rv,vol_ma]).all() or rv <= 0:
            return
        if not self.position and px > s200 and r < 5 and rv > vol_ma:
