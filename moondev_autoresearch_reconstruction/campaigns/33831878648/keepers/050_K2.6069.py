...
class MoonStrategy(Strategy):
    ...
    exit_lookback = 10
    atr_lookback = 20
    stop_atr = 4.0  # increased from 3.5 to allow wider stops

    # Reconstructed sizing controls suggested by the screenshots.
    vol_lookback = 30
    vol_target = 0.35
    f_max = 2.0
    # New: SMA lookback for exit filter
    sma_lookback = 30
    # New: SMA lookback for entry filter
    sma_long_lookback = 60  # increased from 50 to 60
    ...
    def init(self):
        ...
        # New: SMA indicator for exit filter
        self.sma = self.I(
            lambda x: pd.Series(x).rolling(self.sma_lookback).mean().shift(1).to_numpy(),
            self.data.Close
        )
        # New: SMA indicator for entry filter
        self.sma_long = self.I(
            lambda x: pd.Series(x).rolling(self.sma_long_lookback).mean().shift(1).to_numpy(),
            self.data.Close
        )
    ...
    def next(self):
        ...
        rv = float(self.rv[-1])
        sma_val = float(self.sma[-1])
        sma_long_val = float(self.sma_long[-1])

        if not np.isfinite([px, hh, ll, atr, rv, sma_val, sma_long_val]).all() or atr <= 0:
            return

        if not self.position:
            if px > hh and (px - sma_long_val) > 0.5 * atr:
