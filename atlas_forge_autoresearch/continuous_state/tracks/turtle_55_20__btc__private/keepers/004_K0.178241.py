...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.avg_vol = self.I(_vol_mean, self.data.Volume, 20)

    def next(self):
        if len(self.data.Close) < 70:
            return
        px=float(self.data.Close[-1]); hh=float(self.hh[-1]); ll=float(self.ll[-1]); atr=float(self.atr[-1])
        vol=float(self.data.Volume[-1]); avg_vol=float(self.avg_vol[-1])
        if not np.isfinite([px,hh,ll,atr,float(self.rv[-1]), vol, avg_vol]).all() or atr <= 0:
            return
        if not self.position and px > hh and vol > avg_vol:
