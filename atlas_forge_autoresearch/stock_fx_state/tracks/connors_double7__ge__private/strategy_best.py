...
class AtlasStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_sma = self.I(_sma_now, self.data.Volume, 20)
    ...
    def next(self):
        ...
        vol=float(self.data.Volume[-1]); vol_sma=float(self.vol_sma[-1])
        if not np.isfinite([px,s200,lo,hi,rv,vol,vol_sma]).all() or rv <= 0:
            return
        eps=max(1e-12,abs(px)*1e-12)
        if not self.position and px > s200 and px <= lo + eps and vol > vol_sma:
