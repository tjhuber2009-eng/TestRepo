...
class AtlasStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rv_sma = self.I(_sma, self.rv, 30)
    ...
    def next(self):
        ...
        if not np.isfinite([px,hh,ll,sma,atr,float(self.rv[-1])]).all() or atr <= 0:
            return
        if not self.position and px > hh and px > sma and self.sma50[-1] > self.sma50[-2] and self.rv[-1] < self.rv_sma[-1]:
