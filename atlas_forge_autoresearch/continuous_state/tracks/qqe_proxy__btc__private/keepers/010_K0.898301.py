...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rsi_28 = self.I(_rsi, self.data.Close, 28)
    ...
    def next(self):
        if len(self.data.Close) < 60:
            return
        px=float(self.data.Close[-1]); rsi_14=float(self.rsi[-1]); rsi_28=float(self.rsi_28[-1]); rm=float(self.rsi_ma[-1]); atr=float(self.atr[-1])
        if not np.isfinite([px,rsi_14,rsi_28,rm,atr,float(self.rv[-1])]).all() or atr <= 0:
            return
        if not self.position and rsi_14 > 55 and rsi_14 > rsi_28:
            self._buy_with_stop(px, atr, 2.5)
        elif self.position and (rsi_14 < 50 or rsi_14 < rsi_28):
            self.position.close()
