...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_ma = self.I(_vol_mean, self.data.Volume, 20)
    ...
    def next(self):
        ...
        if not np.isfinite([px,ema,sd,rv]).all() or sd <= 0 or rv <= 0:
            return
        z=(px-ema)/sd
        vol_ok = self.data.Volume[-1] > self.vol_ma[-1] if np.isfinite(self.vol_ma[-1]) else False
        if not self.position and z > 0.5 and vol_ok:
