...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.rv_short = self.I(_realized_vol, self.data.Close, 10)

    def next(self):
        if len(self.data.Close) < 70:
            return
        px=float(self.data.Close[-1]); ema=float(self.ema[-1]); atr=float(self.atr[-1])
        rv_long = float(self.rv[-1])
        rv_short = float(self.rv_short[-1])
        if not np.isfinite([px,ema,atr,rv_long,rv_short]).all() or atr <= 0:
            return
        if not self.position and px > ema + 1.5 * atr and rv_short > rv_long:
