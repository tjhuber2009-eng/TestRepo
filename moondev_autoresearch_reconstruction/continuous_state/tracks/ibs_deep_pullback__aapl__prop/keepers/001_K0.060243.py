...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.sma200 = self.I(_sma, self.data.Close, 200)
    ...
    def next(self):
        ...
        sma50_val=self.sma50[-1]; sma200_val=self.sma200[-1]
        if not np.isfinite([px,lo,hi,prior,avg,rv,sma50_val,sma200_val]).all() or hi <= lo or rv <= 0:
            return
        ibs=(px-lo)/(hi-lo)
        lower=prior - 2.5*avg
        if not self.position and px < lower and ibs < 0.30 and sma50_val > sma200_val:
