...
class AtlasStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vol_sma20 = self.I(_sma_now, self.rv, 20)
    ...
    def next(self):
        ...
        vol_sma20=float(self.vol_sma20[-1])
        if not np.isfinite([px,s200,lo,hi,rv]).all() or rv <= 0:
            return
        eps=max(1e-12,abs(px)*1e-12)
        if not self.position and px > s200 and px <= lo + eps and rv < vol_sma20:
