...
def _adx(high, low, close, n):
    val = _adx_now(high, low, close, n)
    return pd.Series(val, dtype=float).shift(1).to_numpy()
...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 70:
            return
        px=float(self.data.Close[-1]); e20=float(self.ema20[-1]); e50=float(self.ema50[-1]); atr=float(self.atr[-1]); adx_val=float(self.adx[-1])
        if not np.isfinite([px,e20,e50,atr,float(self.rv[-1]), adx_val]).all() or atr <= 0:
            return
        trend = e20 > e50 and px > e50
        pullback = abs(px - e20) <= 0.40 * atr
        if not self.position and trend and pullback and adx_val > 20:
