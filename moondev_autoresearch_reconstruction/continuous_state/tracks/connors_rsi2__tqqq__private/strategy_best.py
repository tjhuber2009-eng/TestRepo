...
class MoonStrategy(Strategy):
    ...
    def next(self):
        if len(self.data.Close) < 220:
            return
        px=float(self.data.Close[-1]); s200=float(self.sma200[-1]); s5=float(self.sma5[-1]); r=float(self.rsi2[-1]); rv=float(self.rv[-1]); adx=float(self.adx[-1])
        if not np.isfinite([px,s200,s5,r,rv,adx]).all() or rv <= 0:
            return
        if not self.position and px > s200 and r < 5 and adx > self.adx_threshold:
