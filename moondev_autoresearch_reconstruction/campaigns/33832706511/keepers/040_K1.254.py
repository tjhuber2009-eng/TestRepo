"""
Reconstructed MoonStrategy with volume-filtered breakout entry and volume-confirmed exit.

Keeps the original breakout logic and volatility-targeted sizing, adding a
volume confirmation to reduce false breakouts and false breakdowns.
"""
...
class MoonStrategy(Strategy):
    ...
    exit_lookback = 10
    atr_lookback = 20
    stop_atr = 3.0  # Decreased from 3.5 to tighten stop loss

    # Volume filter.
    vol_ma_lookback = 100  # Increased volume moving average lookback period
    vol_ma_exit_multiplier = 1.6  # Increased volume confirmation for exits
    ...
    def init(self):
        self.hh = self.I(
            _rolling_high, self.data.Close, self.entry_lookback  # Changed to use Close instead of High
        )
        self.ll = self.I(
            _rolling_low, self.data.Close, self.exit_lookback  # Changed to use Close instead of Low
        )
        self.atr = self.I(
            _atr,
            self.data.High,
        ...
        self.vol_ma = self.I(
            _rolling_mean, self.data.Volume, self.vol_ma_lookback
        )
    ...
    def next(self):
        ...
        vol_cur = float(self.data.Volume[-1])

        if not np.isfinite([px, hh, ll, atr, rv, vol_ma, vol_cur]).all() or atr <= 0:
            return

        if not self.position:
            if px > hh and vol_cur > vol_ma * 1.2:  # Increased entry volume threshold
        ...
        elif px < ll - 0.25 * atr and vol_cur > vol_ma * self.vol_ma_exit_multiplier:
            self.position.close()
