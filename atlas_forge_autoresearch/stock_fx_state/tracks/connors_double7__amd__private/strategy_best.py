...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.atr = self.I(_atr, self.data.High, self.data.Low, self.data.Close, self.vol_lookback)
    ...
    def next(self):
        ...
        if not self.position and px > s200 and px <= lo + eps:
            ...
            if units >= 1:
                self.buy(size=units)
                # initialize trailing high on entry bar
                self.trailing_high = float(self.data.High[-1])
        if self.position:
            # update trailing high with current bar's high
            high = float(self.data.High[-1])
            if high > self.trailing_high:
                self.trailing_high = high
            atr_val = float(self.atr[-1])
            if np.isfinite(atr_val) and atr_val > 0:
                stop = self.trailing_high - 3.0 * atr_val
                if float(self.data.Close[-1]) <= stop:
                    self.position.close()
