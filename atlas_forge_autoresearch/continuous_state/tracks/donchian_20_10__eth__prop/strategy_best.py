...
class MoonStrategy(Strategy):
    ...
    def init(self):
        ...
        self.highest_high = None  # track highest high since entry for trailing stop
    ...
    def next(self):
        ...
        if not self.position and px > hh:
            ...
            if np.isfinite(self.data.High[-1]):
                if self.highest_high is None or self.data.High[-1] > self.highest_high:
                    self.highest_high = self.data.High[-1]
            # compute ATR trailing stop
            if self.highest_high is not None and np.isfinite(atr) and atr > 0:
                trailing_stop = self.highest_high - 3.0 * atr
                if px < trailing_stop:
                    self.position.close()
